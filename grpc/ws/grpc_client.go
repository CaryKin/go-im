...
func SendRpc2Client(addr string, messageId, sendUserId, clientId string, code int, message string, data *string) {
	...
	c := NewCommonServiceClient(conn)
	_, err := c.Send2Client(context.Background(), &Send2ClientReq{
		MessageId:  messageId,
...
func CloseRpcClient(addr string, clientId, systemId string) {
	...
	c := NewCommonServiceClient(conn)
	_, err := c.CloseClient(context.Background(), &CloseClientReq{
		SystemId: systemId,
		ClientId: clientId,
	})
	if err != nil {
...
func SendRpcBindGroup(addr string, systemId string, groupName string, clientId string, userId string, extend string) {
	...
	c := NewCommonServiceClient(conn)
	_, err := c.BindGroup(context.Background(), &BindGroupReq{
		SystemId:  systemId,
		GroupName: groupName,
		ClientId:  clientId,
		UserId:    userId,
...
func SendGroupBroadcast(systemId string, messageId, sendUserId, groupName string, code int, message string, data *string) {
	...
	for _, addr := range setting.GlobalSetting.ServerList {
		...
		c := NewCommonServiceClient(conn)
		_, err := c.Send2Group(context.Background(), &Send2GroupReq{
			SystemId:   systemId,
			MessageId:  messageId,
			SendUserId: sendUserId,
			GroupName:  groupName,
...
func SendSystemBroadcast(systemId string, messageId, sendUserId string, code int, message string, data *string) {
	...
	for _, addr := range setting.GlobalSetting.ServerList {
		...
		c := NewCommonServiceClient(conn)
		_, err := c.Send2System(context.Background(), &Send2SystemReq{
			SystemId:   systemId,
			MessageId:  messageId,
			SendUserId: sendUserId,
			Code:       int32(code),
...
func GetOnlineListBroadcast(systemId *string, groupName *string) (clientIdList []string) {
	...
	for _, addr := range setting.GlobalSetting.ServerList {
		go func(addr string) {
			...
			defer conn.Close()
			c := NewCommonServiceClient(conn)
			response, err := c.GetGroupClients(context.Background(), &GetGroupClientsReq{
				SystemId:  *systemId,
				GroupName: *groupName,
			})
			if err != nil {
